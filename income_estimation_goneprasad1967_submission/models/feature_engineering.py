...
class AdvancedFeatureEngineer:
    ...
    def create_financial_ratios(self, df):
        """Create advanced financial ratio features for credit scoring"""
        df_processed = df.copy()
        
        # Credit utilization ratios with better handling
    ...
    def create_interaction_features(self, df):
        ...
        if 'age' in df.columns:
            ...
            # Age bins for non-linear relationships
    ...
    def encode_categorical_features(self, df, fit=True):
        """Enhanced categorical encoding with proper dtype handling"""
        df_processed = df.copy()
            ...
            if col in df.columns:
                if fit:
                    ...
                    if f'{col}_label' in self.encoders:
                        # Handle unseen categories in label encoding
                        try:
    ...
    def advanced_feature_selection(self, X, y, k=150):
        """Advanced feature selection combining statistical and MI methods"""
        if self.feature_selector is None:
            # Combine statistical and mutual information methods
    ...
    def handle_missing_values(self, df):
        ...
        # Numerical columns: use robust imputation
    ...
    def fit_transform(self, df):
        """Complete enhanced feature engineering pipeline for training"""
    ...
    def transform(self, df):
        ...
        # Ensure same feature order and handle missing columns
        missing_cols = set(self.feature_names) - set(df_processed.columns)
        for col in missing_cols:
            if col != 'target':  # Don't add target column during inference
                df_processed[col] = 0
        
        # Keep only the features that were selected during training
        feature_cols = [col for col in self.feature_names if col != 'target']
